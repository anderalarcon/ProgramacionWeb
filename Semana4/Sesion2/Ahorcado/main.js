...
var ocultar=function(refran){//hacemos las lineas
    var refranoculto="";
    for (var i of refran){//Bota las letras altok ; x cada letra
        if(i !=" "){
            refranoculto=refranoculto+"_";
        }else{
...
var mostrarrefranoculto=function(refran){
   // document.querySelector("#refran").innerHTML=refran; con #
    document.getElementById("refran").innerHTML=refran;
}
...
var actualizarImagen=function(){
    document.getElementById("imagen").setAttribute("src","imagenes/Hangman-" 
    +  ++contadorErrores +".png")
...
var teclaapretada=function(evt){
    ...
    verificarcaracterenrefran(caracter,refranSeleccionado);

    if(verificarcaracterenrefran){
        
        descubrircaracterenrefran(caracter,refranSeleccionado,refranOculto);
...
var setearlistener=function(){
    document.getElementById("caracteres_repetidos").addEventListener("keypress",teclaapretada)
}
